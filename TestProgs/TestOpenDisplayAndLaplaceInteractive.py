# -*- coding: utf-8 -*-
"""
TestOpenDisplayAndLaplaceInteractive.py
...
# Axes for Laplace Image
#axFour = fig.add_axes([.3, .2, .7/winAspect, .7])
axLapl = fig.add_subplot(142)
axLapl.axes.set_xticks([])
axLapl.axes.set_yticks([])
axLapl.set_title('Laplace')

# Laplace (exponential) Transform
yyy, xxx = np.mgrid[-11:12,-11:12]
expImg = np.exp(xxx)
expImg[(expImg > 255)] = 0.

laplImg = sig.fftconvolve(imgNp, expImg)

axLapl.imshow(expImg, cmap='gray')
plt.pause(.001)

# Axes for Fourier Image
#axFour = fig.add_axes([.3, .2, .7/winAspect, .7])
axFour = fig.add_subplot(143)
